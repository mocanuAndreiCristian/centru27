...
function Draw() {
    ...
    if (!confettiEnabled) return; // <--- Only draw if enabled
...
// Load confetti state from localStorage
const savedPartyMode = localStorage.getItem("partyMode");
confettiEnabled = savedPartyMode !== "off";
updatePartyModeButton();

partyModeButton.addEventListener("click", () => {
    confettiEnabled = !confettiEnabled;
    localStorage.setItem("partyMode", confettiEnabled ? "on" : "off");
    updatePartyModeButton();
    // Optionally clear canvas when turning off
    if (!confettiEnabled) context.clearRect(0, 0, W, H);
});

function updatePartyModeButton() {
    if (confettiEnabled) {
        partyModeButton.classList.add("active");
        partyModeButton.title = "Dezactivează confetti";
    } else {
        partyModeButton.classList.remove("active");
        partyModeButton.title = "Activează confetti";
    }
}
